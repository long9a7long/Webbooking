...
function submit_add_category(data) {
    ...
    respon.html("");
    var url = "../hotel/submit_add_category";
    var success = function(result) {
        var json_data = $.parseJSON(result);
        if (json_data.status) {
            $('#cate_hotel_table').DataTable().destroy();
            get_list_categorys();
            $('.modal').modal("hide");
          ...
          '<div class="modal-dialog">'+
            '<div class="modal-content">'+
              '<div class="modal-header">'+
                ...
                '<h4 class="modal-title">Sửa danh mục hotel</h4>'+
              '</div>'+
              ...
              '<div class="modal-footer">'+
                ...
                '<button type="submit" class="btn btn-primary" id="submit_edit_cate_hotel" name="submit_add_cate_hotel" onclick="submit_edit_category(' + data.cate_id + ')">Save changes</button>'+
              '</div>'+
            '</div>'+
          '</div>'+
        '</div>';
...
function submit_del_category(cateid) {
    var respon=$("#noti_respon");
    respon.html("");
    if(confirm("Bạn thực sự muốn xóa danh mục hotel này?")){
        var data = {
            cate_id:cateid,
        };
        var url = "../hotel/check_del_category";
        var success = function(result) {
            var json_data = $.parseJSON(result);
            if (json_data.status) {
                $('#cate_hotel_table').DataTable().destroy();
...
function submit_edit_category(data) {
    ...
    var url = "../hotel/check_edit_category";
    var success = function(result) {
        var json_data = $.parseJSON(result);
        if (json_data.status) {
            $('#cate_hotel_table').DataTable().destroy();
